...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // K-quant decode may route to the dq (dequant-float) variant; that decision lives inside ggml_cuda_mul_mat_vec_q.
    ...
    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
        ...
        if (ggml_cuda_can_fuse(cgraph, i, { op, bias_op, op, bias_op, GGML_OP_GLU }, {})) {
            ...
            if (ggml_cuda_should_fuse_mul_mat_vec_q(up)) {
                // SwiGLU gate fusion may route to the dq variant inside mul_mat_vec_q
                // (via the fusion args); no separate dq intercept needed here.
                ggml_cuda_mm_fusion_args_host fusion_data{};
                fusion_data.gate   = gate->src[0];
                fusion_data.glu_op = ggml_get_glu_op(glu);
