...
struct hrx_q4_k_mmqv_a_cache_pack2 {
    int qs[4];
    float d;
    float min;
};
...
static __device__ __forceinline__ void hrx_q4_k_mmqv_load_a_pack2(
        hrx_q4_k_mmqv_a_cache_pack2 * buf_a,
        int buf_idx,
        const hrx_block_q4_K_q8_1_lhs * src0,
        long long row,
        long long kb,
    ...
    const hrx_block_q4_K_q8_1_lhs * block = src0 + row * blocks_per_row + (kb >> 3);
    const int group = static_cast<int>(kb & 7);
    const uint32_t q0 = hrx_q4_k_pack4(block, group, iqs_pair * 2 + 0);
    const uint32_t q1 = hrx_q4_k_pack4(block, group, iqs_pair * 2 + 1);
    buf_a[buf_idx].qs[iqs_pair] = static_cast<int>(q0 | (q1 << 4));
    if (iqs_pair == 0) {
        uint8_t sc = 0;
        uint8_t m = 0;
        hrx_get_scale_min_k4_q4_q8_1(group, block->scales, &sc, &m);
...
extern "C" __global__ void hrx2_mul_mat_vec_q4_k_q8_1_x4_mmql64x32_pack2_wg64_u32(
        const hrx_block_q4_K_q8_1_lhs * src0,
        const hrx_block_q8_1_x4_rhs_q4 * src1,
        float * dst,
        uint32_t k,
    ...
    static_assert(WNITER == 4, "unexpected Q4 pack2 MMQL tile shape");
    static_assert(WSUBM == 64 && WSUBN == 8, "unexpected Q4 pack2 MMQL subtile shape");
    ...
    __shared__ hrx_q4_k_mmqv_a_cache_pack2 buf_a[BM * BK_STEP];
    __shared__ hrx_q8_1_mmqv_b_cache_q4 buf_b[BN * BK_STEP];
    ...
    for (long long kb_base = 0; kb_base < q8_blocks_per_col; kb_base += BK_STEP) {
        ...
        if (full_tile) {
            for (int r = loadc_a; r < BM; r += LOAD_STRIDE_A) {
                #pragma unroll
                for (int k_step = 0; k_step < BK_STEP; ++k_step) {
                    hrx_q4_k_mmqv_load_a_pack2(buf_a, k_step * BM + r, src0, row_base + r, kb_base + k_step, loadr_a, blocks_per_row);
                }
            }
            for (int c = loadc_b; c < BN; c += LOAD_STRIDE_B) {
                #pragma unroll
            ...
            for (int k_step = 0; k_step < BK_STEP; ++k_step) {
                for (int r = loadc_a; r < BM; r += LOAD_STRIDE_A) {
                    const int buf_idx = k_step * BM + r;
                    if (row_base + r < static_cast<long long>(rows)) {
                        hrx_q4_k_mmqv_load_a_pack2(buf_a, buf_idx, src0, row_base + r, kb_base + k_step, loadr_a, blocks_per_row);
                    } else {
                        buf_a[buf_idx].qs[loadr_a] = 0;
                        if (loadr_a == 0) {
                            buf_a[buf_idx].d = 0.0f;
        ...
        #pragma unroll
        for (int k_step = 0; k_step < BK_STEP; ++k_step) {
            hrx_q4_k_mmqv_a_cache_pack2 cache_a[TM];
            #pragma unroll
            for (int cr = 0; cr < TM; ++cr) {
                cache_a[cr] = buf_a[k_step * BM + tiwr * TM + cr];
            }
                    ...
                    for (int cc = 0; cc < TN; ++cc) {
                        ...
                        for (int iqs = 0; iqs < 8; ++iqs) {
                            const uint32_t qpack =
                                (static_cast<uint32_t>(cache_a[cr].qs[iqs >> 1]) >> ((iqs & 1) * 4)) & 0x0F0F0F0Fu;
                            qsum += hrx_sudot4_q4_q8_1(qpack, cache_b[cc].qs[iqs]);
                        }
                        sum[(wsic * TM + cr) * TN + cc] +=
                            cache_a[cr].d * cache_b[cc].d * static_cast<float>(qsum) -
                            cache_a[cr].min * cache_b[cc].s;
...
extern "C" __global__ void hrx2_mul_mat_vec_q4_k_q8_1_x4_vkm64x64_pack2_wg128_u32(
        const hrx_block_q4_K_q8_1_lhs * src0,
        const hrx_block_q8_1_x4_rhs_q4 * src1,
        float * dst,
        uint32_t k,
    ...
    static_assert(WNITER == 8, "unexpected Q4 VKM64x64 WNITER");
    static_assert(WSUBM == 64 && WSUBN == 4, "unexpected Q4 VKM64x64 subtile shape");
...
extern "C" __global__ void hrx2_mul_mat_vec_q4_k_q8_1_x4_vkm64x64_pack2_wg128_w32_u32(
        const hrx_block_q4_K_q8_1_lhs * src0,
        const hrx_block_q8_1_x4_rhs_q4 * src1,
        float * dst,
        uint32_t k,
    ...
    static_assert(WNITER == 8, "unexpected Q4 VKM64x64 W32 WNITER");
    static_assert(WSUBM == 32 && WSUBN == 4, "unexpected Q4 VKM64x64 W32 subtile shape");
