#include "../../../ggml-hrx/kernels/mul_mat_vec_q5_k_q8_1_common.hip.inc"

extern "C" __global__ void hrx2_mul_mat_vec_q5_k_q8_1_x4_mmql128x128_wg256_u32(
        ...
        for (int cr = 0; cr < TM; ++cr) {
            ...
            for (int cc = 0; cc < TN; ++cc) {
                ...
                if (full_tile || (row < static_cast<long long>(rows) && col < static_cast<long long>(cols))) {
                    dst[col * static_cast<long long>(rows) + row] = sum[(wsic * TN + cc) * TM + cr];
                }
            }
        }
    }
...
extern "C" __global__ void hrx2_mul_mat_vec_q5_k_q8_1_x4_vkm64x64_wg128_w32_u32(
        const hrx_block_q5_K_q8_1_lhs * src0,
        const hrx_block_q8_1_x4_rhs_q5 * src1,
        float * dst,
        uint32_t k,
    ...
    static_assert(WNITER == 8, "unexpected Q5 VKM64x64 W32 WNITER");
    static_assert(WSUBM == 32 && WSUBN == 4, "unexpected Q5 VKM64x64 W32 subtile shape");
    ...
    for (long long kb_base = 0; kb_base < q8_blocks_per_col; kb_base += BK_STEP) {
        ...
        for (int k_step = 0; k_step < BK_STEP; ++k_step) {
            ...
            #pragma unroll
            for (int wsic = 0; wsic < WNITER; ++wsic) {
                hrx_q8_1_mmqv_b_cache cache_b[TN];
                #pragma unroll
                for (int cc = 0; cc < TN; ++cc) {
