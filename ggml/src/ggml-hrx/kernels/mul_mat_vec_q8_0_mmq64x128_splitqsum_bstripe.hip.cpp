// Q8_0 x Q8_1 x4 prompt compile probe: preserve the accepted BM64/BN128
// split-qsum shape, but explicitly preload each B-cache iqs stripe before
// consuming it. This tests whether a source-visible B issue-window change can
// move the packed path off the shared first-dot schedule seen by prior probes.
#define hrx_mul_mat_vec_q8_0_f32 hrx_mul_mat_vec_q8_0_mmq64x128_splitqsum_bstripe_unused_f32
#define hrx_mul_mat_vec_q8_0_cols8_f32 hrx_mul_mat_vec_q8_0_cols8_mmq64x128_splitqsum_bstripe_unused_f32
#define hrx_mul_mat_vec_q8_0_q8_1_x4_mmq128x32_wg256_f32 hrx_mul_mat_vec_q8_0_q8_1_x4_mmq128x32_mmq64x128_splitqsum_bstripe_unused_f32
#define hrx_mul_mat_vec_q8_0_add_f32 hrx_mul_mat_vec_q8_0_add_mmq64x128_splitqsum_bstripe_unused_f32
#define hrx_mul_mat_vec_q8_0_add_cols8_f32 hrx_mul_mat_vec_q8_0_add_cols8_mmq64x128_splitqsum_bstripe_unused_f32
#define hrx_mul_mat_vec_q8_0_add_q8_1_x4_mmq128x32_wg256_f32 hrx_mul_mat_vec_q8_0_add_q8_1_x4_mmq128x32_mmq64x128_splitqsum_bstripe_unused_f32
#define hrx_mul_mat_vec_q8_0_add_rows4_cols4_f32 hrx_mul_mat_vec_q8_0_add_rows4_cols4_mmq64x128_splitqsum_bstripe_unused_f32

#include "mul_mat_vec_q8_0.hip.cpp"

extern "C" __global__ void hrx_mul_mat_vec_q8_0_q8_1_x4_mmq64x128_splitqsum_bstripe_wg256_f32(
    ...
    for (long long kb = 0; kb < q8_blocks_per_col; ++kb) {
        ...
        for (int chunk = 0; chunk < COLS_PER_THREAD; chunk += COL_CHUNK) {
            ...
            for (int iqs = 0; iqs < 8; ++iqs) {
                ...
                int bstripe[COL_CHUNK];
                #pragma unroll
                for (int j = 0; j < COL_CHUNK; ++j) {
                    const int col = chunk + j;
                    bstripe[j] = b_qs[col_lane * COLS_PER_THREAD + col][iqs];
                }
                #pragma unroll
                for (int j = 0; j < COL_CHUNK; ++j) {
                    qsum[j] += hrx_sdot4_q8_q8_1(qpack, bstripe[j]);
                }
            }
            ...
            for (int j = 0; j < COL_CHUNK; ++j) {
                ...
                sum[col] += d * __half2float(__ushort_as_half(b_d[c])) * static_cast<float>(qsum[j]);
            }
        }
