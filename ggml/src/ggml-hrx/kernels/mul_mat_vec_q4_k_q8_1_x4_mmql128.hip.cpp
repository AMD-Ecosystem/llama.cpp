...
#ifndef HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_QUAD
...
#ifndef HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_HALF
#define HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_HALF 0
#endif

#ifndef HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_HALF_CR
#define HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_HALF_CR 0
#endif
    ...
    for (long long kb_base = 0; kb_base < q8_blocks_per_col; kb_base += BK_STEP) {
        ...
        for (int k_step = 0; k_step < BK_STEP; ++k_step) {
...
#if HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_OCT
            static_assert(WNITER == 8, "B-oct prefetch expects the full 128-column tile");
            hrx_q8_1_mmql_b_cache_q4 cache_b_oct[WNITER][TN];
            #pragma unroll
            for (int wi = 0; wi < WNITER; ++wi) {
                #pragma unroll
                for (int cc = 0; cc < TN; ++cc) {
                    cache_b_oct[wi][cc] =
                        buf_b[k_step * BN + warp_c * WN + wi * WSUBN + tiwc * TN + cc];
                }
            }
            #pragma unroll
            for (int wsic = 0; wsic < WNITER; ++wsic) {
                #pragma unroll
                for (int cc = 0; cc < TN; ++cc) {
                    const hrx_q8_1_mmql_b_cache_q4 cache_b = cache_b_oct[wsic][cc];
                    #pragma unroll
                    for (int cr = 0; cr < TM; ++cr) {
                        int qsum = 0;
                        #pragma unroll
...
#elif HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_HALF || HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_HALF_CR
            static_assert((WNITER % 4) == 0, "B-half prefetch expects a four-iteration column cluster");
            #pragma unroll
            for (int wsic = 0; wsic < WNITER; wsic += 4) {
                hrx_q8_1_mmql_b_cache_q4 cache_b_half[4][TN];
                #pragma unroll
...
#if HRX_Q4_K_Q8_1_X4_MMQL128_PREFETCH_B_HALF_CR
                #pragma unroll
                for (int cr = 0; cr < TM; ++cr) {
                    #pragma unroll
                    for (int wi = 0; wi < 4; ++wi) {
                        #pragma unroll
                        for (int cc = 0; cc < TN; ++cc) {
                            const hrx_q8_1_mmql_b_cache_q4 cache_b = cache_b_half[wi][cc];
                            int qsum = 0;
                            #pragma unroll
                            for (int iqs = 0; iqs < 8; ++iqs) {
                                const uint32_t qpack =
            ...
            for (int wsic = 0; wsic < WNITER; wsic += 2) {
                hrx_q8_1_mmql_b_cache_q4 cache_b_quad[2][TN];
                #pragma unroll
