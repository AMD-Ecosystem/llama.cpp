...
typedef _Float16 hrx_q6_k_wmma16_wg256_half16_vec __attribute__((ext_vector_type(16)));

static __device__ __forceinline__ uint32_t hrx_q6_k_wmma16_wg256_pack_f16x2(_Float16 lo, _Float16 hi) {
    union {
        _Float16 h[2];
        uint32_t u;
    } pack;
...
static __device__ __forceinline__ _Float16 hrx_q6_k_wmma16_wg256_unpack_f16x2(uint32_t bits, int idx) {
    union {
        uint32_t u;
        _Float16 h[2];
    } pack;
...
static __device__ __forceinline__ hrx_q6_k_wmma16_wg256_half16_vec hrx_q6_k_wmma16_wg256_duplicate_input(
    ...
    const uint32_t p0 = hrx_q6_k_wmma16_wg256_pack_f16x2(x0, x1);
    const uint32_t p1 = hrx_q6_k_wmma16_wg256_pack_f16x2(x2, x3);
    const uint32_t p2 = hrx_q6_k_wmma16_wg256_pack_f16x2(x4, x5);
    const uint32_t p3 = hrx_q6_k_wmma16_wg256_pack_f16x2(x6, x7);
    ...
    hrx_q6_k_wmma16_wg256_half16_vec result;
    result[0] = x0;
    result[1] = x1;
    result[2] = x2;
    result[3] = x3;
    ...
    result[6] = x6;
    result[7] = x7;
    result[8] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s0, 0);
    result[9] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s0, 1);
    result[10] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s1, 0);
    result[11] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s1, 1);
    result[12] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s2, 0);
    result[13] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s2, 1);
    result[14] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s3, 0);
    result[15] = hrx_q6_k_wmma16_wg256_unpack_f16x2(s3, 1);
    return result;
}

static __device__ __forceinline__ _Float16 hrx_q6_k_wmma16_wg256_load_a_value(
...
static __device__ __forceinline__ hrx_q6_k_wmma16_wg256_half16_vec hrx_q6_k_wmma16_wg256_load_a_frag(
        const _Float16 * sh_a,
        int row_tile,
        unsigned int lane) {
    constexpr int BK = 16;
    ...
    return hrx_q6_k_wmma16_wg256_duplicate_input(
...
static __device__ __forceinline__ hrx_q6_k_wmma16_wg256_half16_vec hrx_q6_k_wmma16_wg256_load_b_frag(
        const _Float16 * sh_b,
        int col_tile,
        unsigned int lane) {
    constexpr int BK = 16;
...
static __device__ __forceinline__ void hrx_q6_k_wmma16_wg256_store_acc_f16_row_major(
        float * dst,
        ...
        long long rows,
        long long cols,
        hrx_q6_k_wmma16_wg256_half16_vec acc,
        unsigned int lane) {
...
void hrx_mul_mat_vec_q6_k_wmma16x16_f16acc_wg256_f32(
        const hrx_block_q6_K_q8_1_lhs * src0,
        const float * src1,
        float * dst,
        long long k,
    ...
    for (long long k0 = 0; k0 < k; k0 += BK) {
        for (int idx = static_cast<int>(tid); idx < BM * BK; idx += 256) {
            ...
            sh_a[idx] = row < rows ?
                hrx_q6_k_wmma16_wg256_load_a_value(src0, row, k0 + kk, blocks_per_row) : zero;
        }
        ...
        const hrx_q6_k_wmma16_wg256_half16_vec a =
            hrx_q6_k_wmma16_wg256_load_a_frag(sh_a, wave_row, lane);
        const hrx_q6_k_wmma16_wg256_half16_vec b =
            hrx_q6_k_wmma16_wg256_load_b_frag(sh_b, wave_col, lane);
        acc = __builtin_amdgcn_wmma_f16_16x16x16_f16_w32(a, b, acc, false);

        __syncthreads();
    }

    hrx_q6_k_wmma16_wg256_store_acc_f16_row_major(dst, rows, row0, col0, rows, cols, acc, lane);
}
