...
#ifndef HRX_Q5_K_Q8_1_X4_MMQL128_ISSUE_CR_MAJOR
        ...
        for (int k_step = 0; k_step < BK_STEP; ++k_step) {
...
#if HRX_Q5_K_Q8_1_X4_MMQL128_ISSUE_CR_MAJOR
            #pragma unroll
            for (int cr = 0; cr < TM; ++cr) {
...
#elif HRX_Q5_K_Q8_1_X4_MMQL128_PREFETCH_B_PAIR
            #pragma unroll
            for (int wsic = 0; wsic < WNITER; ++wsic) {
            ...
            for (int wsic = 0; wsic < WNITER; wsic += 2) {
                ...
                for (int wi = 0; wi < 2; ++wi) {
                    #pragma unroll
                    for (int cc = 0; cc < TN; ++cc) {
                        const hrx_q8_1_mmqv_b_cache cache_b = cache_b_quad[wi][cc];
                        #pragma unroll
                        for (int cr = 0; cr < TM; ++cr) {
                            int qsum = 0;
                            #pragma unroll
