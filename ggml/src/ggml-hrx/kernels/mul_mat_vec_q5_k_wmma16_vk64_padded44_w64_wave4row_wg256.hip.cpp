...
static __device__ __forceinline__ void hrx_q5_k_wmma_vk64_wave4row_store_slot(
...
void hrx_mul_mat_vec_q5_k_wmma16x16_vk64_padded44_w64_wave4row_f16acc_wg256_f32(
...
#pragma unroll
        for (int k_tile = 0; k_tile < 2; ++k_tile) {
            a_frag[k_tile] =
                hrx_q5_k_wmma_vk128_load_a_frag_w64_b64asm(
                    sh_a_lds, row_sub, k_tile, lane);
#pragma unroll
            for (int col_sub = 0; col_sub < 4; ++col_sub) {
                b_frag[k_tile][col_sub] =
                    hrx_q5_k_wmma_vk128_load_b_frag_w64_b64asm(
...
#pragma unroll
        for (int slot = 0; slot < 4; ++slot) {
            hrx_q5_k_wmma_vk64_wave4row_store_slot(
