// Q8_0 x Q8_1 x4 prompt compile probe: preserve the accepted
// BM64/BN128/BK2/wave64 packed path, but preload all 32 owned B-cache values
// for an iqs stripe before issuing that stripe's dot products. This is a
// first-dot issue-window probe, not a production route.
#define hrx_mul_mat_vec_q8_0_f32 hrx_mul_mat_vec_q8_0_mmq64x128_splitqsum_bk2_wave64_bfull32_unused_f32
#define hrx_mul_mat_vec_q8_0_cols8_f32 hrx_mul_mat_vec_q8_0_cols8_mmq64x128_splitqsum_bk2_wave64_bfull32_unused_f32
#define hrx_mul_mat_vec_q8_0_q8_1_x4_mmq128x32_wg256_f32 hrx_mul_mat_vec_q8_0_q8_1_x4_mmq128x32_mmq64x128_splitqsum_bk2_wave64_bfull32_unused_f32
#define hrx_mul_mat_vec_q8_0_add_f32 hrx_mul_mat_vec_q8_0_add_mmq64x128_splitqsum_bk2_wave64_bfull32_unused_f32
#define hrx_mul_mat_vec_q8_0_add_cols8_f32 hrx_mul_mat_vec_q8_0_add_cols8_mmq64x128_splitqsum_bk2_wave64_bfull32_unused_f32
#define hrx_mul_mat_vec_q8_0_add_q8_1_x4_mmq128x32_wg256_f32 hrx_mul_mat_vec_q8_0_add_q8_1_x4_mmq128x32_mmq64x128_splitqsum_bk2_wave64_bfull32_unused_f32
#define hrx_mul_mat_vec_q8_0_add_rows4_cols4_f32 hrx_mul_mat_vec_q8_0_add_rows4_cols4_mmq64x128_splitqsum_bk2_wave64_bfull32_unused_f32

#include "mul_mat_vec_q8_0.hip.cpp"

extern "C" __global__ void hrx_mul_mat_vec_q8_0_q8_1_x4_mmq64x128_splitqsum_bk2_wave64_bfull32_wg256_f32(
    ...
    for (long long kb_base = 0; kb_base < q8_blocks_per_col; kb_base += BK_STEP) {
        ...
        for (int k_step = 0; k_step < BK_STEP; ++k_step) {
            ...
            for (int iqs = 0; iqs < 8; ++iqs) {
                int b_full[COLS_PER_THREAD];
                #pragma unroll
                for (int j = 0; j < COLS_PER_THREAD; ++j) {
                    b_full[j] = b_qs[k_step][col_lane * COLS_PER_THREAD + j][iqs];
                }

                const int qpack = hrx_q8_0_pack4(block, iqs);
                #pragma unroll
                for (int j = 0; j < COLS_PER_THREAD; ++j) {
                    qsum[j] += hrx_sdot4_q8_q8_1(qpack, b_full[j]);
                }
            }

            #pragma unroll
            for (int j = 0; j < COLS_PER_THREAD; ++j) {
                const int c = col_lane * COLS_PER_THREAD + j;
                sum[j] += d * __half2float(__ushort_as_half(b_d[k_step][c])) * static_cast<float>(qsum[j]);
            }
        }
