// Q8_0 x Q8_1 x4 prompt probe: BM64/BN64 with cooperative A+B staging.
// This isolates the Vulkan-style A-cache reuse axis from the accepted
// direct-A BM64/BN64 route.
#define hrx_mul_mat_vec_q8_0_f32 hrx_mul_mat_vec_q8_0_mmq64x64_ab_unused_f32
#define hrx_mul_mat_vec_q8_0_cols8_f32 hrx_mul_mat_vec_q8_0_cols8_mmq64x64_ab_unused_f32
#define hrx_mul_mat_vec_q8_0_q8_1_x4_mmq128x32_wg256_f32 hrx_mul_mat_vec_q8_0_q8_1_x4_mmq128x32_mmq64x64_ab_unused_f32
#define hrx_mul_mat_vec_q8_0_add_f32 hrx_mul_mat_vec_q8_0_add_mmq64x64_ab_unused_f32
#define hrx_mul_mat_vec_q8_0_add_cols8_f32 hrx_mul_mat_vec_q8_0_add_cols8_mmq64x64_ab_unused_f32
#define hrx_mul_mat_vec_q8_0_add_q8_1_x4_mmq128x32_wg256_f32 hrx_mul_mat_vec_q8_0_add_q8_1_x4_mmq128x32_mmq64x64_ab_unused_f32
#define hrx_mul_mat_vec_q8_0_add_rows4_cols4_f32 hrx_mul_mat_vec_q8_0_add_rows4_cols4_mmq64x64_ab_unused_f32

#include "mul_mat_vec_q8_0.hip.cpp"

extern "C" __global__ void hrx_mul_mat_vec_q8_0_q8_1_x4_mmq64x64_ab_wg256_f32(
    ...
    if (col_block_base >= cols) {
        return;
    }

    __shared__ int a_qs[BM][8];
    __shared__ unsigned short a_d[BM];
    ...
    for (long long kb = 0; kb < q8_blocks_per_col; ++kb) {
        #pragma unroll
        for (int load_idx = static_cast<int>(tid); load_idx < BM * 8; load_idx += 256) {
            const int r = load_idx >> 3;
            const int iqs = load_idx & 7;
            const long long load_row = row_base + r;
            if (load_row < rows) {
                const hrx_block_q8_0 * block = src0 + load_row * blocks_per_row + kb;
                a_qs[r][iqs] = hrx_q8_0_pack4(block, iqs);
                if (iqs == 0) {
                    a_d[r] = block->d;
                }
            } else {
        ...
        const float d = __half2float(__ushort_as_half(a_d[row_lane]));
        int qsum[COLS_PER_THREAD] = {};

        #pragma unroll
        for (int iqs = 0; iqs < 8; ++iqs) {
            const int qpack = a_qs[row_lane][iqs];
            #pragma unroll
