...
template <int group_base, int col_begin, int col_count, int group_count>
__global__ __launch_bounds__(256, 1)
void q5_array_fullb_phase_repro_kernel(
        const hrx_block_q5_K_wmma_vk128_lhs * src0,
        const float * src1,
        float * dst,
        long long k,
    ...
    constexpr int BK = 32;
    constexpr int SHARED_STRIDE = 44;
    static_assert(col_count >= 1 && col_count <= 4, "unexpected column count");
    static_assert(group_count == col_count * 4, "unexpected group count");
    ...
    const long long blocks_per_row = k / 256;
    const _Float16 zero = static_cast<_Float16>(0.0f);
    hrx_q5_k_wmma_vk128_half8_vec acc[group_count] = {};
...
#pragma unroll
                for (int col_sub = 0; col_sub < 4; ++col_sub) {
                    if (col_sub < col_begin || col_sub >= col_begin + col_count) {
                        q5_repro_consume_frag(b_frag[k_tile][col_sub]);
                    }
                }
            }
            asm volatile("s_waitcnt lgkmcnt(0)\n" ::: "memory");
#pragma unroll
            for (int k_tile = 0; k_tile < 2; ++k_tile) {
#pragma unroll
                for (int local_col = 0; local_col < col_count; ++local_col) {
                    constexpr int row_count = 4;
                    const int col_sub = col_begin + local_col;
#pragma unroll
                    for (int row_sub = 0; row_sub < row_count; ++row_sub) {
                        const int local = local_col * row_count + row_sub;
...
static const char * variant_name(int active_groups) {
    switch (active_groups) {
        case -105: return "array8-tail8-2dispatch";
        case -104: return "array8-tail4-2dispatch";
...
static void launch_variant(
    ...
    switch (active_groups) {
        case -105:
            hipLaunchKernelGGL(
                (q5_array_fullb_phase_repro_kernel<0, 0, 2, 8>),
                grid,
                dim3(256, 1, 1),
                0,
                0,
                ...
                cols);
            hipLaunchKernelGGL(
                (q5_array_fullb_phase_repro_kernel<8, 2, 2, 8>),
                grid,
                dim3(256, 1, 1),
                0,
                0,
        ...
        case -104:
            hipLaunchKernelGGL(
                ...
                cols);
            hipLaunchKernelGGL(
                (q5_array_fullb_phase_repro_kernel<8, 2, 1, 4>),
                grid,
                dim3(256, 1, 1),
                0,
                0,
...
int main() {
    ...
    status |= run_case(64, 33, 256, small, -104);
    status |= run_case(64, 33, 3584, small, -104);
    status |= run_case(64, 64, 3584, small, -104);
    status |= run_case(64, 33, 256, small, -105);
    status |= run_case(64, 33, 3584, small, -105);
    status |= run_case(64, 64, 3584, small, -105);
