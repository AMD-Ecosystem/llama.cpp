...
#pragma unroll
            for (int slot = 0; slot < 4; ++slot) {
                q8_repro_raw_store_slot(dst_rsrc, rows, row_base, col_base, rows, cols, acc, group, slot, lane);
            }
        }
    }
}
...
static bool output_is_active(size_t index, int rows) {
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--mode") == 0 && i + 1 < argc) {
            ...
            std::fprintf(stderr, "usage: %s [--mode array8-fullb|array8-b2|all]\n", argv[0]);
            return 2;
        }
    }
