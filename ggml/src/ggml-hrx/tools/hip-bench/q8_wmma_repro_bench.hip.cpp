...
static int run_motif192_wmma_k2_realdata_fullk_ktilefrag_suite(
    ...
    constexpr int k = 4096;
    int status = 0;
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 128, 128, k, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 128, 129, k, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 129, 128, k, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 1024, 512, k, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 4096, 512, k, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 4096, 513, k, wait_after_load, storebatch);
    return status;
}
...
static int run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(
    ...
    (void) time_fullk_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, 3, false, wait_after_load);
    (void) time_ktilefrag_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, 3, wait_after_load, storebatch);
    const float fullk_ms = time_fullk_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, reps, false, wait_after_load);
    const float ktilefrag_ms = time_ktilefrag_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, reps, wait_after_load, storebatch);
    if (storebatch) {
        std::printf(
...
static int run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_suite(
    ...
    constexpr int k = 4096;
    int status = 0;
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 128, 128, k, 100, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 1024, 512, k, 30, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 4096, 512, k, 20, wait_after_load, storebatch);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 4096, 513, k, 20, wait_after_load, storebatch);
    return status;
}
