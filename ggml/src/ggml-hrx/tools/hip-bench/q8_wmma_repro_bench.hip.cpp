...
static int run_motif192_wmma_k2_realdata_fullk_ktilefrag_suite(const char * label, bool wait_after_load) {
    constexpr int k = 4096;
    int status = 0;
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 128, 128, k, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 128, 129, k, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 129, 128, k, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 1024, 512, k, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 4096, 512, k, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_case(label, 4096, 513, k, wait_after_load);
    return status;
}
...
static int run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(
    ...
    (void) time_fullk_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, 3, false, wait_after_load);
    (void) time_ktilefrag_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, 3, wait_after_load);
    const float fullk_ms = time_fullk_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, reps, false, wait_after_load);
    const float ktilefrag_ms = time_ktilefrag_launches(d_q8.ptr, d_rhs.ptr, d_out.ptr, rows, cols, k, reps, wait_after_load);
    std::printf(
...
static int run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_suite(const char * label, bool wait_after_load) {
    constexpr int k = 4096;
    int status = 0;
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 128, 128, k, 100, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 1024, 512, k, 30, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 4096, 512, k, 20, wait_after_load);
    status |= run_motif192_wmma_k2_realdata_fullk_ktilefrag_timing_case(label, 4096, 513, k, 20, wait_after_load);
    return status;
}
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--mode") == 0 && i + 1 < argc) {
            ...
            std::fprintf(stderr, "usage: %s [--mode motif192-synth-address|motif192-wmma-address|motif192-wmma-waitload-address|motif192-wmma-k2-directwait-waitload-address|motif192-wmma-k2-depwait-waitload-address|motif192-wmma-k2-realdata-k32-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-phase8-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-phase8seq-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-streamfrag-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-ktilefrag-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-colpairfrag-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-accpark-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-accparkfull8-directwait-waitload-address|motif192-wmma-k2-realdata-fullk-timing|motif192-wmma-k2-realdata-fullk-phase8seq-timing|motif192-wmma-k2-realdata-fullk-streamfrag-timing|motif192-wmma-k2-realdata-fullk-ktilefrag-timing|motif192-wmma-k2-realdata-fullk-colpairfrag-timing|motif192-wmma-k2-realdata-fullk-accpark-timing|motif192-wmma-k2-realdata-fullk-accparkfull8-timing|motif192-wmma-direct-address|motif192-wmma-stage16-address|motif192-wmma-stage32-address|motif192-wmma-stage16-waitload-address|motif192-wmma-stage32-waitload-address|array8-fullb|array16-direct-raw|array16-direct-raw-bcopy|array16-direct-raw-abcopy|contract-direct192-raw|contract-direct192-bcopy|contract-direct192-bcopy-hoist|contract-direct192-abcopy|contract-direct192-abcopy-bhoist|contract-bm128-direct192-raw|contract-bm128-direct192-raw-asm|contract-bm128-direct192-bcopy-upper|contract-bm128-direct192-bcopy-upper-asm|contract-bm128-direct192-bcopy-upper-hoist|contract-bm128-direct192-abcopy|contract-bm128-direct192-abcopy-bhoist|contract-bm128-direct192-abcopy-bhoist-asm|contract-phase96-abcopy|phase96-bm128-abcopy|phase96-bm128-abcopy-backendlike|array8-b2|array8-fullb-2phase|array8-fullb-2phase-consume|array8-fullb-2phase-bcopy|array8-fullb-2phase-bcopy-stage|array8-fullb-2phase-abcopy|batched4|batched4-consume|single-group0|single-group0-consume|single-group0-opsel1|single-group0-bcopy-stage|single-group8|single-group8-consume|single-group8-opsel1|single-group8-bmirror0|single-group8-bcopy|single-group8-abcopy|single-group8-bcopy-stage|single-group8-abcopy-stage|single-group8-bcopy-stage-selected|single-group12|single-group12-consume|single-group12-opsel1|single-group12-bmirror0|single-group12-bcopy|single-group12-abcopy|single-group12-bcopy-stage|single-group12-abcopy-stage|single-group12-bcopy-stage-selected|single-group12-abcopy-stage-selected|single-group12-bcopy-stage-selected-acccopy|single-group12-abcopy-stage-selected-acccopy|single-group12-bcopy-stage-selected-regcopy|single-group12-abcopy-stage-selected-regcopy|single-group12-abcopy-dual-stage-raw-first|single-group12-abcopy-dual-stage-stage-first|single-group13|single-group13-consume|single-group13-bcopy-stage-selected|single-group13-abcopy-stage-selected|single-group14-bcopy-stage-selected|single-group14-abcopy-stage-selected|single-group15-bcopy-stage-selected|single-group15-abcopy-stage-selected|remap-c8-s0|remap-c0-s8|remap-c12-s0|remap-c12-s0-bcopy-stage-selected|remap-c12-s0-abcopy-stage-selected|remap-c0-s12|remap-c0-s12-stage-selected|bfrag-dump|all] [--dump-dir <test-backend-ops dump dir>] [--rows N --cols N]\n", argv[0]);
            return 2;
        }
    }
    ...
    if (mode == "phase96-bm128-abcopy") {
        status |= run_case("phase96-bm128-abcopy", 128, 128, k);
        status |= run_case("phase96-bm128-abcopy", 128, 129, k);
        status |= run_case("phase96-bm128-abcopy", 128, 33, k);
        status |= run_case("phase96-bm128-abcopy", 128, 128, 14336);
