...
static __device__ __forceinline__ void q6id_contract_clobber_compact_acc(q6id_contract_u32x4_vec & acc) {
    asm volatile(
        "v_mov_b32 %0, 0\n"
        "v_mov_b32 %1, 0\n"
        "v_mov_b32 %2, 0\n"
...
static __device__ __forceinline__ void q6id_contract_prod_stage_kill_compact_acc(
        uint16_t * sh_prod_stage,
        unsigned int group,
        unsigned int lane,
        q6id_contract_u32x4_vec & acc) {
    __attribute__((address_space(3))) uint16_t * ptr0 =
        (__attribute__((address_space(3))) uint16_t *) (sh_prod_stage + q6id_contract_prod_stage_index(group, 0u, lane));
    __attribute__((address_space(3))) uint16_t * ptr1 =
        (__attribute__((address_space(3))) uint16_t *) (sh_prod_stage + q6id_contract_prod_stage_index(group, 1u, lane));
    __attribute__((address_space(3))) uint16_t * ptr2 =
        (__attribute__((address_space(3))) uint16_t *) (sh_prod_stage + q6id_contract_prod_stage_index(group, 2u, lane));
    __attribute__((address_space(3))) uint16_t * ptr3 =
        (__attribute__((address_space(3))) uint16_t *) (sh_prod_stage + q6id_contract_prod_stage_index(group, 3u, lane));
    asm volatile(
...
static __device__ __attribute__((noinline)) void q6id_contract_prod_stage_compact_acc4(
        uint16_t * sh_prod_stage,
        unsigned int group_base,
        unsigned int lane,
        q6id_contract_u32x4_vec acc0,
...
static __device__ __attribute__((noinline)) void q6id_contract_prod_stage_compact_acc2(
        uint16_t * sh_prod_stage,
        unsigned int group_base,
        unsigned int lane,
        q6id_contract_u32x4_vec acc0,
...
static constexpr int HRX_Q6ID_CONTRACT_RAW_BUFFER_FLAGS_GFX11 = 0x31004000;

static __device__ __forceinline__ __amdgpu_buffer_rsrc_t q6id_contract_make_dst_rsrc(float * dst) {
    return __builtin_amdgcn_make_buffer_rsrc(
        dst,
        static_cast<unsigned short>(0),
        0xffffffffull,
        HRX_Q6ID_CONTRACT_RAW_BUFFER_FLAGS_GFX11);
}

static __device__ __forceinline__ void q6id_contract_raw_buffer_store_f32(
...
static __device__ __forceinline__ void q6id_contract_prod_rawbuf_store_acc(
        __amdgpu_buffer_rsrc_t dst_rsrc,
        unsigned int * counts,
        unsigned int group,
        unsigned int reg,
    ...
    const unsigned int index = col * HRX_Q6ID_CONTRACT_PROD_ROWS + row;
    const _Float16 value = q6id_contract_u16_to_f16(acc[reg] & 0xffffu);
    q6id_contract_raw_buffer_store_f32(dst_rsrc, index, static_cast<float>(value));
    atomicAdd(counts + index, 1u);
}

static __device__ __forceinline__ void q6id_contract_prod_rawbuf_compact_acc(
        __amdgpu_buffer_rsrc_t dst_rsrc,
        unsigned int * counts,
        unsigned int group,
        unsigned int lane,
...
#pragma unroll
    for (unsigned int reg = 0; reg < 4u; ++reg) {
        q6id_contract_prod_rawbuf_store_acc(dst_rsrc, counts, group, reg, lane, acc);
    }
}

static __device__ __forceinline__ void q6id_contract_prod_rawbuf_compact_acc4(
        __amdgpu_buffer_rsrc_t dst_rsrc,
        unsigned int * counts,
        unsigned int group_base,
        unsigned int lane,
        ...
        q6id_contract_u32x4_vec acc2,
        q6id_contract_u32x4_vec acc3) {
    q6id_contract_prod_rawbuf_compact_acc(dst_rsrc, counts, group_base + 0u, lane, acc0);
    q6id_contract_prod_rawbuf_compact_acc(dst_rsrc, counts, group_base + 1u, lane, acc1);
    q6id_contract_prod_rawbuf_compact_acc(dst_rsrc, counts, group_base + 2u, lane, acc2);
    q6id_contract_prod_rawbuf_compact_acc(dst_rsrc, counts, group_base + 3u, lane, acc3);
}
...
extern "C" __global__ __launch_bounds__(256, 1)
void q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper4_probe(float * dst, unsigned int * counts) {
    __shared__ uint64_t sh_frag[12 * 256];
    __shared__ uint16_t sh_prod_stage[16 * 64 * 4];
    ...
    if (wave == 0u) {
...

        {
            ...
            q6id_contract_prod_stage_compact_acc4(
                sh_prod_stage, 0u, lane, acc0, acc1, acc2, acc3);
            q6id_contract_prod_stage_compact_acc4(
                sh_prod_stage, 4u, lane, acc4, acc5, acc6, acc7);
        }

        {
            ...
            q6id_contract_prod_stage_compact_acc4(
                sh_prod_stage, 8u, lane, acc0, acc1, acc2, acc3);
            q6id_contract_prod_stage_compact_acc4(
                sh_prod_stage, 12u, lane, acc4, acc5, acc7, acc6);
        }
    }
...
extern "C" __global__ __launch_bounds__(256, 1)
void q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper4_rawbuf_probe(float * dst, unsigned int * counts) {
    __shared__ uint64_t sh_frag[12 * 256];

    const unsigned int tid = __builtin_amdgcn_workitem_id_x();
    const unsigned int lane = tid & 63u;
    const unsigned int wave = tid >> 6u;
    const __amdgpu_buffer_rsrc_t dst_rsrc = q6id_contract_make_dst_rsrc(dst);
    ...
    if (wave == 0u) {
...

        {
            ...
            q6id_contract_prod_rawbuf_compact_acc4(
                dst_rsrc, counts, 0u, lane, acc0, acc1, acc2, acc3);
            q6id_contract_prod_rawbuf_compact_acc4(
                dst_rsrc, counts, 4u, lane, acc4, acc5, acc6, acc7);
        }

        {
            ...
            q6id_contract_prod_rawbuf_compact_acc4(
                dst_rsrc, counts, 8u, lane, acc0, acc1, acc2, acc3);
            q6id_contract_prod_rawbuf_compact_acc4(
                dst_rsrc, counts, 12u, lane, acc4, acc5, acc7, acc6);
        }
    }
}

extern "C" __global__ __launch_bounds__(256, 1)
void q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper2_probe(float * dst, unsigned int * counts) {
    __shared__ uint64_t sh_frag[12 * 256];
    __shared__ uint16_t sh_prod_stage[16 * 64 * 4];
    ...
    if (wave == 0u) {
...

        {
            ...
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 0u, lane, acc0, acc1);
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 2u, lane, acc2, acc3);
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 4u, lane, acc4, acc5);
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 6u, lane, acc6, acc7);
        }

        {
            ...
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 8u, lane, acc0, acc1);
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 10u, lane, acc2, acc3);
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 12u, lane, acc4, acc5);
            q6id_contract_prod_stage_compact_acc2(sh_prod_stage, 14u, lane, acc7, acc6);
        }
    }
...
extern "C" __global__ __launch_bounds__(256, 1)
void q6_id_subgroup_contract_prodaddr_radv_issue_compact_clobber_probe(float * dst, unsigned int * counts) {
    __shared__ uint64_t sh_frag[12 * 256];
    __shared__ uint16_t sh_prod_stage[16 * 64 * 4];
    ...
    if (wave == 0u) {
...

        {
            ...
            HRX_Q6ID_CONTRACT_PROD_STAGE_COMPACT_ACC8_INLINE(
                0u, acc0, acc1, acc2, acc3, acc4, acc5, acc6, acc7);
            q6id_contract_clobber_compact_acc(acc0);
            q6id_contract_clobber_compact_acc(acc1);
            q6id_contract_clobber_compact_acc(acc2);
            q6id_contract_clobber_compact_acc(acc3);
            q6id_contract_clobber_compact_acc(acc4);
            q6id_contract_clobber_compact_acc(acc5);
            q6id_contract_clobber_compact_acc(acc6);
            q6id_contract_clobber_compact_acc(acc7);
        }
...
extern "C" __global__ __launch_bounds__(256, 1)
void q6_id_subgroup_contract_prodaddr_radv_issue_compact_storekill_probe(float * dst, unsigned int * counts) {
    __shared__ uint64_t sh_frag[12 * 256];
    __shared__ uint16_t sh_prod_stage[16 * 64 * 4];
    ...
    if (wave == 0u) {
        ...
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 0u, lane, acc0);
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 1u, lane, acc1);
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 2u, lane, acc2);
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 3u, lane, acc3);
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 4u, lane, acc4);
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 5u, lane, acc5);
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 6u, lane, acc6);
        q6id_contract_prod_stage_kill_compact_acc(sh_prod_stage, 7u, lane, acc7);
...
static double run_prod_kernel(const std::string & name, int reps, float * d_dst, unsigned int * d_counts) {
    ...
    for (int i = 0; i < reps; ++i) {
        ...
        } else if (name == "prodaddr-radvissuecompact-helper4") {
            q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper4_probe<<<1, 256>>>(d_dst, d_counts);
        } else if (name == "prodaddr-radvissuecompact-helper4-rawbuf") {
            q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper4_rawbuf_probe<<<1, 256>>>(d_dst, d_counts);
        } else if (name == "prodaddr-radvissuecompact-helper2") {
            q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper2_probe<<<1, 256>>>(d_dst, d_counts);
        ...
        } else if (name == "prodaddr-radvissuecompact-clobber") {
            q6_id_subgroup_contract_prodaddr_radv_issue_compact_clobber_probe<<<1, 256>>>(d_dst, d_counts);
        } else if (name == "prodaddr-radvissuecompact-storekill") {
            q6_id_subgroup_contract_prodaddr_radv_issue_compact_storekill_probe<<<1, 256>>>(d_dst, d_counts);
...
int main(int argc, char ** argv) {
    ...
    clear_prod_buffers(d_prod_dst, d_prod_counts);
    q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper4_probe<<<1, 256>>>(d_prod_dst, d_prod_counts);
    HIP_CHECK(hipGetLastError());
    HIP_CHECK(hipDeviceSynchronize());
    const prod_validate_result prod_radvissuecompact_helper4 = validate_prod(d_prod_dst, d_prod_counts, false);

    clear_prod_buffers(d_prod_dst, d_prod_counts);
    q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper4_rawbuf_probe<<<1, 256>>>(d_prod_dst, d_prod_counts);
    HIP_CHECK(hipGetLastError());
    HIP_CHECK(hipDeviceSynchronize());
    const prod_validate_result prod_radvissuecompact_helper4_rawbuf = validate_prod(d_prod_dst, d_prod_counts, false);

    clear_prod_buffers(d_prod_dst, d_prod_counts);
    q6_id_subgroup_contract_prodaddr_radv_issue_compact_helper2_probe<<<1, 256>>>(d_prod_dst, d_prod_counts);
    HIP_CHECK(hipGetLastError());
    HIP_CHECK(hipDeviceSynchronize());
    const prod_validate_result prod_radvissuecompact_helper2 = validate_prod(d_prod_dst, d_prod_counts, false);
    ...
    clear_prod_buffers(d_prod_dst, d_prod_counts);
    q6_id_subgroup_contract_prodaddr_radv_issue_compact_clobber_probe<<<1, 256>>>(d_prod_dst, d_prod_counts);
    HIP_CHECK(hipGetLastError());
    HIP_CHECK(hipDeviceSynchronize());
    const prod_validate_result prod_radvissuecompact_clobber = validate_prod(d_prod_dst, d_prod_counts, false);

    clear_prod_buffers(d_prod_dst, d_prod_counts);
    q6_id_subgroup_contract_prodaddr_radv_issue_compact_storekill_probe<<<1, 256>>>(d_prod_dst, d_prod_counts);
    HIP_CHECK(hipGetLastError());
    HIP_CHECK(hipDeviceSynchronize());
    const prod_validate_result prod_radvissuecompact_storekill = validate_prod(d_prod_dst, d_prod_counts, false);
    ...
    const double prod_radvissuecompact_helper4_us = run_prod_kernel("prodaddr-radvissuecompact-helper4", reps, d_prod_dst, d_prod_counts);
    const double prod_radvissuecompact_helper4_rawbuf_us =
        run_prod_kernel("prodaddr-radvissuecompact-helper4-rawbuf", reps, d_prod_dst, d_prod_counts);
    const double prod_radvissuecompact_helper2_us = run_prod_kernel("prodaddr-radvissuecompact-helper2", reps, d_prod_dst, d_prod_counts);
    ...
    const double prod_radvissuecompact_clobber_us = run_prod_kernel("prodaddr-radvissuecompact-clobber", reps, d_prod_dst, d_prod_counts);
    const double prod_radvissuecompact_storekill_us = run_prod_kernel("prodaddr-radvissuecompact-storekill", reps, d_prod_dst, d_prod_counts);
    ...
    std::printf("prodaddr_radvissuecompact_helper4,%d,%.6f\n",
        prod_radvissuecompact_helper4.ok ? 1 : 0,
        prod_radvissuecompact_helper4_us);
    std::printf("prodaddr_radvissuecompact_helper4_rawbuf,%d,%.6f\n",
        prod_radvissuecompact_helper4_rawbuf.ok ? 1 : 0,
        prod_radvissuecompact_helper4_rawbuf_us);
    std::printf("prodaddr_radvissuecompact_helper2,%d,%.6f\n",
        prod_radvissuecompact_helper2.ok ? 1 : 0,
        prod_radvissuecompact_helper2_us);
    std::printf("prodaddr_radvissuecompact_scoped,%d,%.6f\n",
        prod_radvissuecompact_scoped.ok ? 1 : 0,
        prod_radvissuecompact_scoped_us);
    std::printf("prodaddr_radvissuecompact_clobber,%d,%.6f\n",
        prod_radvissuecompact_clobber.ok ? 1 : 0,
        prod_radvissuecompact_clobber_us);
    std::printf("prodaddr_radvissuecompact_storekill,%d,%.6f\n",
        prod_radvissuecompact_storekill.ok ? 1 : 0,
        prod_radvissuecompact_storekill_us);
    ...
    std::printf("prodaddr_radvissuecompact_helper4,%d,missed=%u,duplicated=%u,max_count=%u\n",
        prod_radvissuecompact_helper4.ok ? 1 : 0,
        prod_radvissuecompact_helper4.missed,
        prod_radvissuecompact_helper4.duplicated,
        prod_radvissuecompact_helper4.max_count);
    std::printf("prodaddr_radvissuecompact_helper4_rawbuf,%d,missed=%u,duplicated=%u,max_count=%u\n",
        prod_radvissuecompact_helper4_rawbuf.ok ? 1 : 0,
        prod_radvissuecompact_helper4_rawbuf.missed,
        prod_radvissuecompact_helper4_rawbuf.duplicated,
        prod_radvissuecompact_helper4_rawbuf.max_count);
    std::printf("prodaddr_radvissuecompact_helper2,%d,missed=%u,duplicated=%u,max_count=%u\n",
        prod_radvissuecompact_helper2.ok ? 1 : 0,
        prod_radvissuecompact_helper2.missed,
        prod_radvissuecompact_helper2.duplicated,
        prod_radvissuecompact_helper2.max_count);
    ...
    std::printf("prodaddr_radvissuecompact_clobber,%d,missed=%u,duplicated=%u,max_count=%u\n",
        prod_radvissuecompact_clobber.ok ? 1 : 0,
        prod_radvissuecompact_clobber.missed,
        prod_radvissuecompact_clobber.duplicated,
        prod_radvissuecompact_clobber.max_count);
    std::printf("prodaddr_radvissuecompact_storekill,%d,missed=%u,duplicated=%u,max_count=%u\n",
        prod_radvissuecompact_storekill.ok ? 1 : 0,
        prod_radvissuecompact_storekill.missed,
        prod_radvissuecompact_storekill.duplicated,
        prod_radvissuecompact_storekill.max_count);
    ...
    return (direct_ok && staged_ok && loaddeep_ok && minstore_ok && banked_ok && bankedcompact_ok &&
            ...
            prod_radvissuecompact_helper4.ok && prod_radvissuecompact_helper4_rawbuf.ok &&
            prod_radvissuecompact_helper2.ok &&
            prod_radvissuecompact_scoped.ok && prod_radvissuecompact_clobber.ok &&
            prod_radvissuecompact_storekill.ok) ? 0 : 1;
}
