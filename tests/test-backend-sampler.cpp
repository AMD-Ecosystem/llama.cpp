...
static std::vector<const backend_test_case *> collect_tests_to_run(const std::string & requested) {
    ...
    if (!requested.empty()) {
        ...
        for (const auto & test : BACKEND_TESTS) {
            ...
            // top-p logits, mid-stream top-k reconfigure), so they fail on HIP.
            // Skip until TOP_K is supported on the ROCm backend.
            if (test.name == "set_sampler" || test.name == "mixed" || test.name == "top_p") {
